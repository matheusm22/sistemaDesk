...
if (isset($_POST['usuario']) || isset($_POST['senha']) || isset($_POST['matricula'])) {
    ...
    if (strlen($_POST['senha']) < 6 or strlen($usuario) < 6) {
        echo "<label style='color: #fff;'>Erro: Senha ou usuário com menos de 6 caracteres!</label>";

    } else {
    
    if(strlen($matricula) != 4){ 
        
        echo "<label style='color: #fff;'>Número de matricula inválido!, EX:0000</label>";
       ...
       if ($verifica_matricula->num_rows > 0) {
              ...
              echo "<label style='color: #fff;'>Erro: Número de matricula em uso, Tente outro!</label>";
       } else {
       ...
       if ($verifica_usuario->num_rows > 0) {
           ...
           echo "<label style='color: #fff;'>Erro: Nome de usuário em uso, Tente outro!</label>";
    ...
    // INSERINDO NO BANCO DE DADOS:
        ...
        $sql = mysqli_query($conexao, "SELECT * FROM tb_acessos WHERE usuario = '$usuario' ORDER BY id_usuario ASC");
            ...
            if ($sql->num_rows == 0 || $usuario == '' || $senha == '' || $matricula == '') {
                     echo "<label style='color: #ffff;'>Erro: Usuário não cadastrado, tente novamente!</label>";
        } else {
